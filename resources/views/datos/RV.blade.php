				...
				<table id="example2" class="table table-bordered table-hover">
					...
					<tbody>
						...
						<tr>
							<td>{{$rv->id}}</td>
							<td>{{$rv->ot_id}}</td>
							<td>{{ Carbon\Carbon::parse($rv->fecha)->format('Y-m-d') }}</td>
							<td>{{ Carbon\Carbon::parse($rv->fecha)->format('Y-m-d') }}</td>
							<td>{{$rv->find($rv->ot_id)->oT->cliente}}</td>
							<td>
								<a id="boton-{{ $rv->id }}" class="btn btn-flat btn-block btn-primary boton" role="button" name="{{ $rv->id }}" data-toggle="modal">Ver Items</a>
							</td>
						</tr>
						@endforeach
					</tbody>
...
<script>
	$(function () {		
		...
		$('.boton').click(function () {
			...
			$.ajax({
			    url : '{{ action('RVController@mostrarItems')}}', // la URL para la petición	    
			    ...
			    success : function(respuesta) {
			    	...
			    	for (var i = 0; i < respuesta.id.length; i++) {
			    		$("#table-modal > #tabla-body").prepend('<tr><td>' + respuesta.id[i] + '</td><td>' + respuesta.codigo[i] + '</td><td>' + respuesta.descripcion[i] + '</td><td>' + respuesta.cantidad[i] + '</td><td>' + respuesta.nota[i] + '</td></tr>');
			    	}
