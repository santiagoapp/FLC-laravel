...
<script>
	...
	$(function () {
		...
		$('.editar').click(function () {
			...
			var autoriza = $('#tr-'+id_boton+'>td:nth-of-type(4)').html();
			var cargo = $('#tr-'+id_boton+'>td:nth-of-type(5)').html();

			$('#equipo option[value='+ equipo +']').attr("selected", true);
			$('#motivo').val(motivo)
			$('#autoriza').val(autoriza)
			$('#cargo').val(cargo)
			$('#hidden').val(id_boton)
		...
		function editarBaja(){
			$.ajax({
				...
				success: function (data) {
					...
					$('#tr-'+data.id+'>td:nth-of-type(4)').html(data.autoriza);
					$('#tr-'+data.id+'>td:nth-of-type(5)').html(data.cargo);
				},
				error: function (data) {
					console.log('Error:', data);
				}
